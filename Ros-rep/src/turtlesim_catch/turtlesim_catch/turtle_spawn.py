...
class Turtlespawn(Node):
    def __init__(self):
        ...
        self.turtle_conta = 0
        self.alive_turtles = []
        self.viva_turtle_publisher = self.create_publisher(TurtleArray, "turtle_viva")
    ...
    def publish_alive_turtles(self):
        ...
        self.viva_turtle_publisher.publish(msg)
    ...
    def spawn_new_turtle(self):
        self.turtle_conta += 1
        name = f"{self.turtle_prefix}_{self.turtle_conta}"
    ...
    def call_spawn_service(self, turtle_name, x, y, theta):
        ...
        while not self.spawn_client.wait_for_service(1.0):
            self.get_logger().warn("Esperando pelo serviço /spawn...")
    ...
    def callback_spawn(self, future, request: Spawn.Request):
        try:
            ...
            if response.name != "":
                self.get_logger().info(f"Sucesso! Nova tartaruga: {response.name}")
        except Exception as e:
            self.get_logger().error(f"Falha ao chamar serviço: {e}")
            new_turtle = Turtle()
...
def main(args=None):
    ...
    node = Turtlespawn()
    node.call_spawn_service("primeira_tartaruga", 5.0, 5.0, 0.0)
    rclpy.spin(node)
    rclpy.shutdown() 
